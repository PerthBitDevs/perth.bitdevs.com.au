...
Start from `templates/manifest.json.tmpl` for new current or upcoming months. Replace every placeholder before running `just site-check`.
...
Tracked authoring templates live in `templates/`. They are copy sources, not deployed assets and not a build system.

Use them as the starting point for new monthly content:

```bash
cp templates/manifest.json.tmpl YYYY-MM/manifest.json
cp templates/month-hub.html.tmpl YYYY-MM/index.html
cp templates/slide-deck.html.tmpl YYYY-MM/topic-name.html
```
...
| Template | Use |
|----------|-----|
| `templates/manifest.json.tmpl` | Current-month event metadata, topics, source refs, auxiliary page ownership, and landing summary |
| `templates/month-hub.html.tmpl` | Current-month hub with TV-friendly cards and source footer |
| `templates/slide-deck.html.tmpl` | Standard slide deck with canonical runtime, footer nav, Topics link, Esc handling, accessible disclosure controls, CSS tokens, and TV scale |
| `templates/auxiliary-visual-page.html.tmpl` | Non-slide visual explainer with Topics link, Esc handling, and accessible step controls |
| `templates/archive-page.html.tmpl` | Historical scrollable topic archive |
...
2. **Creates** the topic slide HTML file at `YYYY-MM/topic-name.html` by copying `templates/slide-deck.html.tmpl`
...
Create `YYYY-MM/index.html` as a hub page linking to all topic pages. Start from `templates/month-hub.html.tmpl`:
...
Start from `templates/archive-page.html.tmpl` if another historical archive page is needed.
